...
<form>
    <div class="mb-3">
      ...
      <input type="hidden" name="category" value="@if(isset($category)) {{$category->id}} @endif">

    </div>

@if(isset($category))
    <button type="button" class="btn btn-primary" id="editCategory" data-id="{{$category->id}}">Update</button>
    @else
    ...
    $("#editCategory").on('click',function(){
        ...
        $.ajax({
            ...
            if(err.status == 500){
                var errors = err.responseJSON.errors;
                $('#message').empty()
                $('#message').fadeIn().append("<p class='alert alert-warning alert-dismissible fade show'>"+errors+"<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button><p>");
            }
