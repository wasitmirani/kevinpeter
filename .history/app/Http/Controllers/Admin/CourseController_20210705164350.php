...
class CourseController extends Controller
{
    public function index(){
        ...
        if(Auth::user()->role_id == '1') {
            $courses = Course::with('teachers')->with('category')->orderby('role_id','desc')->get();
        }

        if(Auth::user()->role_id == '2') {
            $courses = Course::with('teachers')->with('category')->where('role_idid','=',Auth::user()->id)->orderby('id','desc')->get();

        }

        if(Auth::user()->role_id == '3') {
            $courses = Course::with('teachers')->with('category')->where('role_id','=',Auth::user()->id)->orderby('id','desc')->get();
